...
(define concerts
  (list
   (list "band@example.com" "Rockin' the Night Away" "2025-08-01" "50" #t)
   (list "band@example.com" "An Evening of Jazz" "2025-08-15" "40" #t)
   (list "band@example.com" "Summer Pop Festival" "2025-09-10" "60" #f)))
...
(define (add-concert! band-email title date price)
  (set! concerts (cons (list band-email title date price #t) concerts)))
...
(define (update-concert! idx new-title new-date new-price new-available)
  (when (and (integer? idx) (>= idx 0) (< idx (length concerts)))
    (set! concerts
          (for/list ([c (in-list concerts)]
                     [i (in-naturals)])
            (if (= i idx)
                (list (first c) new-title new-date new-price new-available)
                c)))))
        ...
        (body
         ...
         (ul
           ,@(map (lambda (indexed-concert)
                    ...
                    (define display-text (string-append title " - " date " - $" price))
                    `(li ,(if available?
...
(define (update-listing-handler request idx user)
  ...
  (when (and concert (string=? (first concert) band-email))
    (let* ([bindings (request-bindings request)]
           [title (extract-binding/default 'title bindings "")]
           [date (extract-binding/default 'date bindings "")]
           [price (extract-binding/default 'price bindings "")]
           [available (let ([avail-binding (assoc 'available bindings)])
                        (and avail-binding (string=? (cdr avail-binding) "true")))])
      (when (not (or (string-blank? title) (string-blank? date) (string-blank? price)))
        (update-concert! idx title date price available))))
  (band-dashboard user))
